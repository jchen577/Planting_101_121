...
export class MainMenu extends Phaser.Scene {
    ...
    create() {
        ...
        const playButton = this.add.text(
            ...
            200,
            "Play",
            { font: "32px Arial", fill: "#ffffff", backgroundColor: "#0000ff", padding: 10 }
        )
        .setOrigin(0.5)
        .setInteractive()
        .on("pointerdown", () => {
        ...
        const settingsButton = this.add.text(
            ...
            300,
            "Settings",
            { font: "32px Arial", fill: "#ffffff", backgroundColor: "#ff0000", padding: 10 }
        )
        .setOrigin(0.5)
        .setInteractive()
    ...
    openSettingsMenu() {
        ...
        const settingsMenu = this.add.container(this.cameras.main.centerX, this.cameras.main.centerY);

        const menuBg = this.add.rectangle(0, 0, 400, 300, 0xffffff).setOrigin(0.5);
        settingsMenu.add(menuBg);

        const menuTitle = this.add.text(0, -100, "Select Language", { font: "24px Arial", fill: "#000000" }).setOrigin(0.5);
        ...
        languages.forEach((lang, index) => {
            const langButton = this.add.text(
                0,
                -50 + index * 50,
                lang.name,
                { font: "20px Arial", fill: "#000000", backgroundColor: "#dddddd", padding: 10 }
            )
            .setOrigin(0.5)
            .setInteractive()
            .on("pointerdown", () => {
