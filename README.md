...
When game is deployed from Github, the player can install the game onto their smartphone by hitting the share button on their browser. The game can be installed by them hitting the "add to home screen" button which will download the app onto their phone to be playable on their smartphone device. We used the help of Brace to achieve the installable functionality. We used the PWA route to simplify the installation process. 
