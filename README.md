...
### F0+F1+F2
Our implementations for F0, F1, and F2 software requirements have stayed consistent. The game still has the same tile structure, gameplay, and autosave/manual functionality as before. We also still have our YAML, external DSL, and internal DSL working for the current iteration of the game. The current game is also fully in Javascript for the platform switch. In terms of code quality, from the beginning, we have tried our best to compartmentalize our code so that features and the functions/structures that correspond with them can easily be understood and edited. This has helped a lot in terms of streamlining the coding pipeline as it decreased the likelihood of conflicts.

### [F3.a] Internationalization
Our game has three language options which are displayed under the "Settings" option on our game's home screen. Once a language option is selected, the text of the entire game will be translated to that language. We did this by creating JSON files for each language (i.e. lang_jp.json). Each file included all the game objects and texts that need to be translated along with a corresponding translation. Within our code, instead of hard-coded strings, we make a call to the JSON file and parse it to fetch the proper translated string. This method was simple yet effective thanks to the limited number of strings that needed to be localized.

Something to note: a tool/plugin called "i18n" was also considered as an option to aid us in localizing. However, as mentioned above, due to the small scale of our game, simply using a JSON file to map translations worked just fine.

### [F3.b] Localization
Our game is localized to English, Japanese, and Arabic. Japanese for the logographic requirement and Arabic for the right-to-left script requirement. Each file has the game objects and game keys containing localized translations for the interface text of the game. Our use of JSON files to achieve this allows us to dynamically switch the UI between the three languages implemented. The player can switch between the languages in the main menu of the game via the "Settings" tab.

The Japanese translations were done by Ethan, who studied Japanese in college and double-checked with a Japanese dictionary: [Jisho](https://jisho.org/). The Arabic translations were done with the help of an online translator [DeepL](https://www.deepl.com/en/translator).

### [F3.c] Mobile Installation
We used the PWA route to simplify the installation process. When game is deployed from Github, the player can install the game onto their smartphone by hitting the share button on their browser. The game can be installed by hitting the "add to home screen" button which will download the app onto their phone to be playable on their smartphone device. We used the help of Brace to achieve the installable functionality.

### [F3.d] Mobile Play (Offline)
We needed to change the grid size and scale by minimizing it so our game can be playable on a smartphone device. We also implemented move arrows and a "plant" button for the player so they can navigate their way around the game without a keyboard. Our game runs offline without any issues so we didn't need to make any changes to implement that. 

## Reflections 
We made a lot of reconsiderations for this round of development as we were having issues with the build of the deployment environment. Our concern was that using the Deno runtime was causing issues with our deployment environment. The game did not properly deploy our assets into GitHub which caused our game installations to not work. We ended up using Brace to help refactor our code to help fix our build issue which alleviated our plan of making a new repository and switching to a live server. As our game evolved we had to think about how we could most easily implement localization for the player. We decided the easiest way to implement this is to just allow players to switch their preferred language in a settings page (similar to many games today). We also needed to scale the playing grid of our game to fit with the mobile format which made us refactor much of our HUD to account for both browser and mobile.<br />
Many of these changes were made easier thanks to the pipeline we had set up beforehand, with multiple GitHub branches and good code quality (especially through modularity).<br />
In terms of our group's roles, we loosely stuck with them, and our teamwork evolved into a steady pattern of certain members introducing and adding a tool/function into our game while other members would build off of this tool to seamlessly incorporate it into our game. Additionally, it was common for certain members to become "experts" in certain areas of the game they worked on (i.e. the UI, plants, map generation, etc.) due to the modularity of our game.
